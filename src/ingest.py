...
def load_flights(path: str = CSV_PATH) -> pd.DataFrame:
    ...
    # Drop junk column if present and empty
    if "Unnamed: 2" in df.columns:
        if df["Unnamed: 2"].isna().all() or (df["Unnamed: 2"].astype(str).str.strip() == "").all():
    ...
    if {"From", "To"} <= set(df.columns):
        df["route"] = (
            df["From"].astype(str).str.replace("\xa0", " ", regex=False)
            + "->" +
    ...
    elif "STD_MinOfDay" in df.columns:
    ...
    if "DepartureDelayMin" in df.columns and "dep_delayed_15" not in df.columns:
